...
<head>
  ...
  <?php
    ...
    $stmt = $conexao->prepare("select id, nome, descricao, fotoevento, data_inicio_evento, hora_inicio_evento from evento");
    $stmt->execute();
        ...
        for($j = $count; $j < $count+3; $j++){
          ...
          <a href=<?php echo "evento.php?evento=".juntarNomeEvento($resultado[$j]["nome"]); ?>>
            ...
            <?php  echo "<img src='imagens/eventos/".$resultado[$j]["fotoevento"]."' width='300px' height='200px' >"; ?>
                       ...
                       <p class="card-text camp-time"><small><em><?php echo dataProUsuario($resultado[$j]["data_inicio_evento"])." - ".horarioEvento($resultado[$j]["hora_inicio_evento"]); ?></em></small></p>
                        <a href=<?php echo "evento.php?evento=".juntarNomeEvento($resultado[$j]["nome"]); ?> class="blue-button">Visualizar</a>
                    </div>
                </div>
            </a>
  <?php } ?>
